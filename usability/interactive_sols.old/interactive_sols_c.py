...
def process_solution(nb_cells_to_process=1):
    ...
    <script>
    IPython.notebook.kernel.execute('w=12')
        if (init_cell_sols!==undefined) {{
        init_cell_sols({0})
        IPython.notebook.kernel.execute('w=cbx_id; cbx_id = int(cbx_id)+1; z=cbx_id; ')
    ...
    # get_cbx_id() #At first execution, get the current (initial) value of cbx_id.
    # Then, each time a checkbox is created,
