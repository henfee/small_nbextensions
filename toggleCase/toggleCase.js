...
define(function(require, exports, module) {
    ...
    function initialize(conf) {
      ...
      //    Jupyter.notebook.metadata.case_toggle);
            ...
            toggleHotkey(conf);
            showToolbar();
            // load_css('./toggleCase.css') //we don't need css in this extension
            main_function(conf);
        })
        return conf
    }
    ...
    function load_css(name) { // append a css file 
    ...
    function load_notebook_extension() {
        ...
        if (Jupyter.notebook._fully_loaded) {
            ...
            console.log(log_prefix + "Notebook fully loaded -- case_toggle initialized ")
            conf = initialize(conf);
        } else {
            console.log(log_prefix + "Waiting for notebook availability")
            $([Jupyter.events]).on("notebook_loaded.Notebook", function() {
                console.log(log_prefix + "case_toggle initialized (via notebook_loaded)")
                conf = initialize(conf);                
            })
        }
